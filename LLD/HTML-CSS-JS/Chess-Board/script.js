(function () {
    const rookNextCells = [
      { row: -1, column: 0, cell: -8 },
      { row: 0, column: 1, cell: 1 },
      { row: 1, column: 0, cell: 8 },
      { row: 0, column: -1, cell: -1 }
    ];
    const bishopNextCells = [
      { row: -1, column: -1, cell: -9 },
      { row: -1, column: 1, cell: -7 },
      { row: 1, column: 1, cell: 9 },
      { row: 1, column: -1, cell: 7 }
    ];
    ...
    function revertCells() {
      if (revertCellsArray.length) {
        revertCellsArray.forEach(function (revertCell) {
          squares[revertCell.cellId - 1].style.backgroundColor =
            revertCell.revertBackgroundColor;
        });
        revertCellsArray = [];
      }
    }
    ...
    function fillValidCells(row, column, cellId, nextCells) {
      ...
      revertCellsArray.push({
        cellId,
        revertBackgroundColor: elem.style.backgroundColor
      });
      ...
      nextCells.forEach((nextCell) => {
        ...
        while (tempRow > 0 && tempRow < 9 && tempColumn > 0 && tempColumn < 9) {
          ...
          revertCellsArray.push({
            cellId: tempCellId,
            revertBackgroundColor: tempCellElement.style.backgroundColor
          });
    ...
    chessPiece.addEventListener("change", (e) => {
      switch (e.target.value) {
        case "rook":
          nextCells = rookNextCells;
          break;
        case "bishop":
          nextCells = bishopNextCells;
          break;
        case "queen":
          nextCells = [...rookNextCells, ...bishopNextCells];
          break;
      }
    });
